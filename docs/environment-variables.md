...
Most relay behavior — logging level, audit retention, rate limits, connection limits, WebSocket compression, NIP-11 metadata, optional NIP plugins under the `nips` map (`{ "<plugin-id>": { "enabled", "settings" } }`), core NIP-42 under `nip42`, shutdown timeouts, etc. — is configured in the JSON file referenced by `CONFIG_PATH`. Legacy configs with `nips.enabled` as an integer array are migrated automatically on load (`config_version`). Listen ports and the SQLite file path can additionally be overridden at process start by the variables above (applied after JSON load, then the merged config is validated).
