...
| `CONGEE_DATA_DIR` | libSQL database directory | Optional. When set and `database.type` is empty, `sqlite`, or `turso`, sets `database.dsn` to `<dir>/congee.db` and `database.meta_dsn` to `<dir>/congee-meta.db` (after path cleaning). Empty/`sqlite` types are then rewritten to `turso` on boot. Ignored when `database.type` is `postgres`. The official container image sets this to `/data` by default; mount a volume there for persistence. |
...
NIP-11 `self` is derived from the relay signing identity in `relay.secrets.json`. The optional `nip11.admin_pubkey` is a separate 32-byte hex administrator contact key, published as NIP-11 `pubkey`; leave it empty to omit the contact identity. The old `nip11.pubkey` config value was the relay's own key, so it is ignored and removed on the next config save. The response derives `limitation.max_message_length`, `max_subscriptions`, `max_subid_length`, and (when enabled) `default_limit` from enforced config values. It reports `auth_required: false` because ordinary requests do not require authentication; specific operations can still require NIP-42 authentication. Congee does not advertise a `max_limit` or `max_event_tags` cap because it does not enforce those caps.
