...
Most relay behavior — logging level, audit retention, rate limits, connection limits, WebSocket compression, NIP-11 metadata, `nips.enabled`, shutdown timeouts, etc. — is configured in the JSON file referenced by `CONFIG_PATH`. Listen ports and the events/meta file paths can additionally be overridden at process start by the variables above (applied after JSON load, then the merged config is validated).
