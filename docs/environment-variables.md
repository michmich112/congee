...
| `CONGEE_RELAY_PORT` | Relay HTTP/WebSocket listen port | Optional. When set (decimal integer), overrides `relay.port` from JSON after load. Must be between 1 and 65535. |
| `CONGEE_ADMIN_PORT` | Admin HTTP listen port | Optional. When set, overrides `admin.port` from JSON after load. Must be between 1 and 65535. Ignored for binding when `ENABLE_ADMIN_UI` is not enabled, but the value still overrides the in-memory config. |
