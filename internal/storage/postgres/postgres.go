...
func applyPostgresAuditLogFilters(q *bun.SelectQuery, query storage.AuditQuery) *bun.SelectQuery {
	...
	if sql, args := storage.AuditDetailKindSuffixMatchOr(false, query.Kinds); sql != "" {
		q = q.Where(sql, args...)
	}
	return q
}
...
func (s *Store) QueryAuditLog(ctx context.Context, query storage.AuditQuery) ([]storage.AuditEntry, error) {
	...
	q := applyPostgresAuditLogFilters(s.db.NewSelect().Model(&rows), query).Order("created_at DESC")
	if query.Limit > 0 {
...
func (s *Store) CountAuditLog(ctx context.Context, query storage.AuditQuery) (int64, error) {
	q := applyPostgresAuditLogFilters(s.db.NewSelect().Model((*storage.AuditLogRow)(nil)), query)
	n, err := q.Count(ctx)
	return int64(n), err
}
...
// SaveWSConnectionSession implements storage.Store.
func (s *Store) SaveWSConnectionSession(ctx context.Context, e storage.WSConnectionSession) (int64, error) {
	...
	_, err := s.db.NewInsert().Model(&row).Returning("id").Exec(ctx)
	if err != nil {
		return 0, err
	}
	return row.ID, nil
...
// QueryWSConnectionSessions implements storage.Store.
...
func (s *Store) PurgeWSConnectionSessionsBefore(ctx context.Context, olderThanUnix int64) (int64, error) {
	res, err := s.db.NewDelete().Model((*storage.WSConnectionSessionRow)(nil)).
