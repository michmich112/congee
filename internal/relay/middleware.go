...
type EventValidator interface {
	Validate(ctx context.Context, ev *nostr.Event) error
}

// EventValidatorFunc adapts a function to EventValidator.
type EventValidatorFunc func(ctx context.Context, ev *nostr.Event) error

// Validate implements EventValidator.
func (f EventValidatorFunc) Validate(ctx context.Context, ev *nostr.Event) error {
	return f(ctx, ev)
}
...
func (c *ValidatorChain) Validate(ctx context.Context, ev *nostr.Event) error {
	c.mu.RLock()
	list := c.validators
	c.mu.RUnlock()
	for _, v := range list {
		if err := v.Validate(ctx, ev); err != nil {
			return err
		}
	}
	return nil
